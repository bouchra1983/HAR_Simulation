...
NUM_FEATURES = 561 # Number of features in HAR dataset
...
# Chemins des données
DATA_DIR = "/home/ubuntu/har_data/federated_har_data/"
        ...
        for round_num in range(1, self.num_rounds + 1):
            ...
            for client_id in selected_clients:
                ...
                if X_train is None or y_train is None:
                    print(f"Skipping client {client_id} due to data loading error.")
                    continue
                
                # Check for sufficient classes before training
                if len(np.unique(y_train)) < 2:
                     print(f"Skipping client {client_id} training: not enough classes ({len(np.unique(y_train))}).")
                     continue # Skip training if not enough classes
                     
                training_time = self.train_client(client_id, X_train, y_train)
                
                # Only include clients that were successfully trained (or attempted)
                if hasattr(self.client_models[client_id], 'coef_'):
                     client_weights[client_id] = len(X_train)
                     valid_clients_in_round.append(client_id)
                else:
                     print(f"Client {client_id} model not fitted after training attempt.")

            # Aggregate models only if there are valid clients from the round
            if valid_clients_in_round:
                self.aggregate_models(valid_clients_in_round, client_weights)
            else:
                 print(f"Round {round_num}: No valid client models to aggregate.")
                 # If no aggregation happens, global model remains unchanged. Evaluation might use old model.

            # Evaluate the global model (even if it wasn't updated, to track progress)
            if self.global_model is not None:
                eval_metrics = self.evaluate_model(X_test_global, y_test_global)
            else:
                 # Handle case where global model never got initialized
                 eval_metrics = {'accuracy': 0.0, 'precision': 0.0, 'recall': 0.0, 'f1': 0.0, 'loss': np.inf}
                 
            comm_cost = self.calculate_communication_cost(valid_clients_in_round) # Cost based on valid clients
...
class CFL_KBS(FedAvg):
    ...
    def aggregate_models(self, selected_clients, client_weights):
        ...
        for cluster_id, weight in cluster_total_clients.items():
            if cluster_id in self.cluster_models and hasattr(self.cluster_models[cluster_id], 'coef_'):
...
def load_client_train_data(client_id):
    file_path = os.path.join(DATA_DIR, f"client_{client_id}_train.csv")
    try:
...
def load_global_test_data():
    ...
    for client_id in range(1, NUM_CLIENTS + 1):
        X_test, y_test = load_client_test_data(client_id)
        if X_test is not None and y_test is not None:
             all_X_test.append(X_test)
             all_y_test.append(y_test)
        else:
             print(f"Skipping client {client_id} in global test set due to missing data.")
    if not all_X_test:
         print("Error: No test data loaded for global evaluation.")
         return None, None
    try:
        X_test_global = np.vstack(all_X_test)
        y_test_global = pd.concat(all_y_test, ignore_index=True)
        return X_test_global, y_test_global
    except ValueError as e:
         print(f"Error stacking test data: {e}. Check shapes.")
         # Print shapes for debugging
         # for i, arr in enumerate(all_X_test):
         #     print(f"Client {i+1} test shape: {arr.shape}")
         return None, None
...
def run_all_simulations():
    ...
    encryptions = ['AES', 'RSA', 'ECC', 'HE', 'SMPC', 'DP', 'PE']
    results = []
    ...
    results_df.to_csv(f"{OUTPUT_DIR}simulation_summary.csv", index=False)
    print("\nToutes les simulations sont terminées. Résumé enregistré dans simulation_summary.csv")
    return results_df
